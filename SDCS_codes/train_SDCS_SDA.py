...
class SDA(nn.Module):
    def __init__(self,A):
        ...
        self.linear1 = nn.Linear(33 * 33, 33 * 33)
        self.linear2 = nn.Linear(33 * 33, 33 * 33)
        self.linear3 = nn.Linear(33 * 33, 33 * 33)
    ...
    def forward(self, inputs, sampling_matrix_mask):
        ...
        outputs = torch.reshape(torch.squeeze(outputs), [-1, 33 * 33])

        outputs = nn.ReLU()(self.linear1(outputs))
        outputs = nn.ReLU()(self.linear2(outputs))
        outputs = nn.ReLU()(self.linear3(outputs))
...
if __name__ == "__main__":
    ...
    EpochNum = 100
    batch_size = 64
    results_saving_path = "../../results_c4/SDA"
